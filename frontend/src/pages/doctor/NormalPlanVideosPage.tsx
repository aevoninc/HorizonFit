...
import { GoogleDriveVideoHelper } from "@/components/GoogleDriveVideoHelper";
import {
  ZoneVideo,
  HorizonGuideVideo,
  ZONE_DEFINITIONS,
...
import { getEmbedUrl, getVideoUrlInstructions } from "@/lib/videoEmbedHelper";
...
export const NormalPlanVideosPage: React.FC = () => {
  ...
  const handleSubmit = async () => {
    ...
    try {
      ...
      const embedResult = getEmbedUrl(formData.videoUrl);
      
      if (!embedResult.isValid) {
        toast({
          title: "Invalid Video URL",
          description: embedResult.error || "The video URL format is not supported.",
          variant: "destructive",
        });
      ...
      const finalVideoUrl = embedResult.embedUrl;
      ...
      if (activeTab === "zone-videos") {
        const videoData = {
          ...
          videoUrl: formData.type === "video" ? finalVideoUrl : undefined,
          pdfUrl: formData.type === "pdf" ? finalVideoUrl : undefined,
  ...
  return (
    ...
    >
      ...
      {/* Help Section for Google Drive Videos */}
      <Alert className="border-blue-200 bg-blue-50">
        <AlertCircle className="h-4 w-4 text-blue-600" />
        <AlertDescription className="text-blue-800">
          <strong>📹 Adding Videos from Google Drive?</strong> Upload to Google Drive,
          share the file with "Anyone with the link", then paste the sharing link below.
          The system will automatically convert it for embedding!
          {" "}
          <a
            ...
            <div className="space-y-2">
              ...
              <Alert className="mt-2 border-blue-200 bg-blue-50">
                ...
                <AlertDescription className="ml-2 text-sm text-blue-800">
                  <strong>For Google Drive:</strong> Share the file, then paste
                  the link (e.g.,{" "}
                  <code className="text-xs bg-white px-1 rounded">
                    https://drive.google.com/file/d/ABC123/view
                  </code>
                  ). Videos will be auto-converted for embedding.
                </AlertDescription>
              </Alert>
            </div>
