...
import { getEmbedUrl } from "@/lib/videoEmbedHelper";
import {
  HorizonGuideVideo,
        ...
        <DialogContent className="max-w-4xl p-0 overflow-hidden">
          ...
          <div className="aspect-video bg-black">
            {selectedVideo?.videoUrl ? (
              <VideoPlayerContent video={selectedVideo} />
            ) : (
...
// Helper component to render video or error
const VideoPlayerContent: React.FC<{ video: any }> = ({ video }) => {
  const embedResult = getEmbedUrl(video.videoUrl);
  ...
  return (
    <div className="flex h-full items-center justify-center text-white">
      <div className="text-center">
        <BookOpen className="mx-auto h-16 w-16 opacity-50" />
        <p className="mt-4 text-lg">Video Unavailable</p>
        <p className="mt-1 text-sm opacity-70">
          {embedResult.error || 'Unable to load this video'}
        </p>
      </div>
    </div>
  );
