...
const sendPasswordResetOtpEmail = async (recipient, userName, otpCode) => {
    const subject = 'Your HorizonFit Password Reset OTP';
    const htmlBody = passwordResetOtpTemplate(userName, otpCode);
    const textBody = `Hello ${userName}, your password reset OTP is ${otpCode}. It is valid for 10 minutes.`;
    await sendEmail(recipient, subject, textBody, htmlBody);
};
