...
const authLogin = asyncHandler(async (req, res) => {
    ...
    if (user && (await user.matchPassword(password))) {
        ...
        const { accessToken, refreshToken } = await setTokens(res, user._id, user.role); 
        ...
        await RefreshToken.create({
            ...
            userAgent,
            ipAddress,
            expiresAt: new Date(Date.now() + (parseInt(process.env.JWT_REFRESH_SECRET_EXPIRY, 10) * 24 * 60 * 60 * 1000)), // Matches cookie maxAge
        });
